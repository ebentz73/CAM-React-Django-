...
from app.models import InputDataSet, Scenario, Input, EvalJob, InputPage, NodeResult, AnalyticsSolution, Model
from app.serializers import NodeResultSerializer
from app.utils import upload_file_to_gcp
...
def start(request):
    ...
    container = client.create_container('trunavconsolecore:latest',
                                        detach=True,
                                        environment={'EVALJOBDEF_URL': 'https://skjdhfkdj.com/api/evaljob',
                                                     'RESULTS_URL': 'https://sdfjwlksf.com/api/results'})
...
def upload_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            upload_file_to_gcp(f.name, f.read())
            return render(request, 'index.html')
    else:
        form = UploadFileForm()
